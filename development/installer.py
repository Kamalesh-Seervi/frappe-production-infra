...
def create_site_in_bench(args):
    if "mariadb" == args.db_type:
        cprint("Set db_host", level=3)
        subprocess.call(
            ["bench", "set-config", "-g", "db_host", "mariadb-master"],
            cwd=os.getcwd() + "/" + args.bench_name,
        )
        new_site_cmd = [
            "bench",
            "new-site",
            f"--db-host=mariadb-master",  # Should match the compose service name
            f"--db-type={args.db_type}",  # Add the selected database type
            f"--no-mariadb-socket",
            f"--db-root-password=123",  # Replace with your MariaDB password
            f"--admin-password={args.admin_password}",
            f"--install-app=erpnext",  # Install the ERPNext app
            args.site_name,
        ]
        subprocess.call(new_site_cmd, cwd=os.getcwd() + "/" + args.bench_name)
    else:
        cprint("Set db_host", level=3)
        subprocess.call(
